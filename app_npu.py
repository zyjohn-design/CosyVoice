...
# ══════════════════════════════════════════════════════════════
#  华为 NPU 910B 适配
#  通过 torch_npu + transfer_to_npu 自动映射 .cuda() → .npu()
# ══════════════════════════════════════════════════════════════
...
def _load_wav_16k(audio_path: str) -> torch.Tensor:
    ...
    [NPU适配] 用 soundfile 替代 torchaudio.load, 避免 torchcodec 依赖。
    """
    data, sr = sf.read(audio_path, dtype='float32')
    # numpy → tensor, 确保 2D (channels, samples)
    if data.ndim == 1:
...
def audio_tensor_to_bytes(t: torch.Tensor, sr: int) -> bytes:
    ...
    [NPU适配] 用 soundfile 替代 torchaudio.save, 避免 torchcodec。
    """
    if t.ndim == 1:
        t = t.unsqueeze(0)
    # tensor → numpy: (channels, samples) → (samples, channels)
    data = t.cpu().numpy().T if t.shape[0] > 1 else t.squeeze(0).cpu().numpy()
    buf = io.BytesIO()
...
def save_to_wavs_dir(audio: torch.Tensor, sr: int, tag: str = "") -> tuple:
    ...
    # [NPU适配] 用 soundfile 保存, 避免 torchaudio.save 调 torchcodec
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"正在加载 CosyVoice 模型: {MODEL_DIR} (设备: {DEVICE_TYPE}) ...")
    t0 = time.time()

    # ── [NPU] patch load_wav: 用 soundfile 替代 torchaudio.load ──
    # torchaudio 2.9.0 默认用 torchcodec, NPU PyTorch 不兼容
    import cosyvoice.utils.file_utils as _fu_module
    def _patched_load_wav(wav, target_sr, min_sr=16000):
        if isinstance(wav, str):
            import soundfile as _sf
            data, sample_rate = _sf.read(wav, dtype='float32')
            speech = torch.from_numpy(data).unsqueeze(0) if data.ndim == 1 else torch.from_numpy(data.T)
            if speech.shape[0] > 1:
        ...
        return speech
    _fu_module.load_wav = _patched_load_wav
    logger.info("已 patch load_wav (soundfile 模式)")
    ...
    # patch frontend 模块的 load_wav 引用
    try:
        import cosyvoice.cli.frontend as _fe_module
        if hasattr(_fe_module, 'load_wav'):
            _fe_module.load_wav = _patched_load_wav
    except Exception:
        pass

    # ── [NPU] patch HiFi-GAN 的 _istft: NPU 不支持 torch.istft ──
    # aclnnUnfoldGrad 算子在 910B 上缺失, torch.istft 会报错
    # 解决: 将 istft 计算放到 CPU 上执行, 结果搬回 NPU
    # istft 是最后一步波形合成, 数据量小, CPU 开销可忽略
    if DEVICE_TYPE == "npu":
        try:
            import cosyvoice.hifigan.generator as _hifi_module
            _original_istft = _hifi_module.HiFTGenerator._istft
            def _cpu_istft(self, magnitude, phase):
                """将 istft 放到 CPU 执行, 绕过 NPU 算子缺失"""
                orig_device = magnitude.device
                real = magnitude * torch.cos(phase)
                img = magnitude * torch.sin(phase)
                # 搬到 CPU 做 istft
                result = torch.istft(
                    torch.complex(real.cpu(), img.cpu()),
                    self.istft_params["n_fft"],
                    self.istft_params["hop_len"],
                    self.istft_params["n_fft"],
                    torch.hann_window(self.istft_params["n_fft"]).cpu(),
                )
                return result.to(orig_device)
            _hifi_module.HiFTGenerator._istft = _cpu_istft
            logger.info("已 patch HiFi-GAN._istft (CPU fallback)")
        except Exception as e:
            logger.warning(f"patch HiFi-GAN._istft 失败: {e}")
...
async def tts_zero_shot(
    ...
    if save_speaker:
        ...
        if spk_name:
            ...
            # [NPU适配] 用 soundfile 保存
            sf.write(wav_path, t.squeeze(0).cpu().numpy(), 16000, subtype='PCM_16')
    ...
    if DEVICE_TYPE == "npu":
        try:
            device_info = {
                ...
                "memory_total_mb": round(torch.npu.get_device_properties(0).total_memory / 1024**2),
                "memory_used_mb": round(torch.npu.memory_allocated(0) / 1024**2),
            }
        except Exception:
            device_info = {"device_type": "Huawei NPU", "status": "available"}
    elif torch.cuda.is_available():
