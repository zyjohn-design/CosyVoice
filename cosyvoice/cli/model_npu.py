# ============================================================
# CosyVoice model.py — 华为 NPU 910B 适配版
# 
# 与 GPU 版的差异:
#   1. load_vllm: 兼容 vLLM 0.9 (V0) 和 vLLM 0.17 (V1)
#      - V1 需要 runner='generate', enforce_eager=True
#      - NPU 上 gpu_memory_utilization=0.6 (GPU 版 0.2)
#   2. 注册模型用字符串格式, 支持 spawn 子进程
#   3. is_text_generation_model patch 支持 vLLM 0.17
#
# 使用方式:
#   docker-compose.npu.yml 中挂载:
#     - ./model_npu.py:/workspace/CosyVoice/cosyvoice/cli/model.py
# ============================================================

# 此文件只包含需要修改的 load_vllm 方法
# 在容器中使用 sed 或 python 脚本替换原始 model.py 中的 load_vllm 方法
# 或直接作为参考, 手动修改 model.py
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def load_vllm(self, model_dir):
        ...
        # ── 注册 CosyVoice2 到 vLLM ModelRegistry ──
        import cosyvoice.vllm.cosyvoice2  # 触发注册
 
        from packaging.version import parse as vparse
        import vllm as _vllm_pkg
        vllm_ver = vparse(_vllm_pkg.__version__.split('+')[0])
 
        from vllm import EngineArgs, LLMEngine
 
        if vllm_ver >= vparse("0.11.0"):
            # ── vLLM 0.11+ / 0.17+ (V1 引擎, NPU 需要 spawn) ──
            engine_args = EngineArgs(
                model=model_dir,
                skip_tokenizer_init=True,
                enable_prompt_embeds=True,
                runner='generate',
                enforce_eager=True,           # NPU 不支持 CUDAGraph
                gpu_memory_utilization=0.6,   # NPU 需要更多显存给 KV cache
